...
開発段階で発生した「VAリーク（Loop 9で停止）」や「最適化スキップ（12ms問題）」は、これらの混同が原因であった。
...
この結合処理により、バラバラに解放された 64MB のブロック群も、最終的には元の巨大な 16GB のブロックへと自然に復元される。これにより、長時間稼働後の巨大確保（Frag Test の最後など）も失敗することなく成功する。
...
* **リスク**: `Frag Test` のように、プールが 5GB の断片を抱え込んだ状態で、新たに 2GB の連続確保が必要になった場合、合計 7GB となり VRAM 6GB を超過してクラッシュする。
