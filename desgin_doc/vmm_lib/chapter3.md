...
#### 2. 仮想アドレスプールの必要性：OSリソース消費の抑制
都度予約（Reserve per Malloc）方式を採用した場合、アプリケーションのメモリ確保回数に比例して `cuMemAddressReserve` が発行される。
将来の拡張性（リサイズ）を見越して各領域ごとに十分な仮想アドレス（例: 32GB）を予約しようとすると、小さなメモリ確保（例: 64MB）が大量に繰り返された場合、実際に使用する物理メモリ量とは無関係に、仮想アドレスの消費総量が指数関数的に増大してしまう。

これは以下の技術的課題を引き起こすため、本ライブラリでは採用していない。

* **OS 管理領域の圧迫**: `cuMemAddressReserve` は物理メモリ（VRAM）を消費しないが、**ホストメモリ（RAM）上のページテーブルや管理ハンドル** を消費する。無制限な予約はホストメモリを圧迫し、OS のスラッシングを引き起こす原因となる。
* **WDDM の制限**: 特に Windows (WSL2) 環境では、GPU ドライバが管理できるリソースハンドル数に厳格な上限があり、多数の小規模な `Reserve` はこれに抵触しやすい。

VA プール戦略は、OS に対する `Reserve` 発行回数を「1回」に固定することで、これらの問題を根本から解決し、OS のリソース制限に依存しないスケーラビリティを確保している。
